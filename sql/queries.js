...
exports.CatalogAllProducts = 
  'select ptc.category_id, po.id as product_id, p.name, po.product_color, p.price + po.price_increase as price, poi.`path`, p.slug, po.color_slug '+
  `
  from product p 
...
exports.productImages = (name, color) =>{
  const query = 'select poi.id as image_id, poi.`path`, poi.main_image ' +
  `from product_options po 
  left join product_options_image poi 
  on poi.option_id = po.id 
  left join product p 
